...
public class StudyController {
	...
	public String passCheck2Post(String pwd, Model model) {
		//입력문자가 영문 소문자일 경우는 대문자로 변경처리(연산 시 자리수 over 때문)
		pwd = pwd.toUpperCase();
		...
		for(int i=0; i<pwd.length(); i++) {
			intPwd = (long) pwd.charAt(i);  // A=65
			strPwd += intPwd; // 위에서 바뀐거 누적
		}
		...
		//암호화를 위한 EOR 연산
		encPwd = intPwd ^ key;  //암호화 : DB에 저장
		strPwd = String.valueOf(encPwd); //숫자를 문자로 바꿔서 db에 저장
		model.addAttribute("encPwd", encPwd);  // 암호화된 문자
		// 복호화
		intPwd = Long.parseLong(strPwd);
		decPwd = intPwd ^ key; //복호화 
		strPwd = String.valueOf(decPwd); // 문자
		
		// 복호화된 문자형식의 아스키코드값을 2개씩 분류하여 실제 문자로 변환
		String result = "";
		char ch;
	...
	public String operatorInputOkPost(OperatorVO vo) {
		...
		if(vo2 !=null) return "redirect:/msg/opertorCheckNo"; // 아이디 중복
		
		// 아이디가 중복되지 않았으면 비밀번호를 암호화하여 저장. service객체에서 처리
		studyService.setOperatorInputOk(vo);
		
		return "redirect:/msg/opertorInputOk";
	}
...
//		vos = studyService.getCityStringListStr(dodo);
//		return vos;
		return studyService.getCityStringListStr(dodo);
	}
...
//		return vos;
	...
	//BCryptPasswordEncoder 암호화 방식 연습
	...
	public String securityCheckPost(String pwd) {
		...
		pwd = "Encoding : "+encPwd + " / 원본비밀번호 : "+ pwd;
	...
	public String mailFormPost(MailVO vo) {
		try {
			...
			// 메일 보관함에 회원이 보내온 메세지를 모두 저장시켜둔다.
			...
			content += "<p>방문하기 : <a href='http://192.168.50.115:9090/javagreenS'>javagreenJ사이트</a></p>";
			content += "<hr>";
